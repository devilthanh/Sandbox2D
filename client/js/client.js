...
drawScaleImage = function(img, x, y, width, height){
	c.drawImage(img, Math.floor(x*scaleX), Math.floor(y*scaleY), Math.ceil(width*scaleX), Math.ceil(height*scaleY));
}	

drawScaleCropImage = function(img, sx, sy, sw, sh, x, y, width, height){
	c.drawImage(img, sx, sy, sw, sh, Math.floor(x*scaleX), Math.floor(y*scaleY), Math.ceil(width*scaleX), Math.ceil(height*scaleY));
}	
...
loadMap = function(){
	for(var y=0; y<mapHeight; y++)
		for(var x=0; x<mapWidth; x++){
			map[y][x].sd = 0;
			if(map[y][x].type==1 || map[y][x].type==5)
				map[y][x].sd = 2;
			if(map[y][x].type==2)
				map[y][x].sd = 1;
			...
			if(map[y][x].sd==0){
				...
				if(x-1>=0)
					sd += Math.floor(map[y][x-1].sd/2)*2;
				if(y-1>=0) 
					sd += Math.floor(map[y-1][x].sd/2);
				if((x-1>=0)&&(y-1>=0))
					sd += Math.floor(map[y-1][x-1].sd/2)*4;
...
connection.addEventListener('message', event => {
	...
	if(header==="load"){
		...
		test = firebase.database().ref().child(name + '/rot');
	}else if(header==="chatUpdate"){
		var chatBox = document.getElementById("chatBox");
		chatBox.innerHTML = chatBox.innerHTML + "<div style=\"color:#ff0000\"><b>" + data.name + ": <span style=\"color:#000000\">" + data.msg + "</span></b></div>";
		...
		for(var y=startY; y<endY; y++)
			for(var x=startX; x<endX; x++){
				...
				if(style==="tileshadow_obstacle" && map[y][x].type==2)
					drawScaleImage(Img.tileShadow, x*mapTileWidth - PLAYERS[playerID].x + canvas.width/2 - 2, y*mapTileHeight  - PLAYERS[playerID].y + canvas.height/2 - 2, 36, 36);		
				else if(style==="tileshadow_wall" && map[y][x].type==1)
					drawScaleImage(Img.tileShadow, x*mapTileWidth - PLAYERS[playerID].x + canvas.width/2 - 2, y*mapTileHeight  - PLAYERS[playerID].y + canvas.height/2 - 2, 36, 36);				
				else if(style==="floor" && (map[y][x].type==0 || map[y][x].type >= 10))
					drawScaleCropImage(Img.tileset, Img.tiles[tileId].x, Img.tiles[tileId].y, Img.tiles[tileId].w, Img.tiles[tileId].h, x*mapTileWidth - PLAYERS[playerID].x + canvas.width/2, y*mapTileHeight  - PLAYERS[playerID].y + canvas.height/2, mapTileWidth, mapTileHeight);	
...
playersUpdate = function(){
	for(i in PLAYERS){
		...
		c.drawImage(Img.playerShadow, PLAYERS[i].x - PLAYERS[playerID].x + canvas.width/2-Img.playerShadow.width/2, PLAYERS[i].y - PLAYERS[playerID].y + canvas.height/2-Img.playerShadow.height/2, Img.playerShadow.width, Img.playerShadow.height);
		c.translate(PLAYERS[i].x - PLAYERS[playerID].x + canvas.width/2, PLAYERS[i].y - PLAYERS[playerID].y + canvas.height/2);
		c.rotate(rot);
		...
		for(var y=startY; y<endY; y++)
			for(var x=startX; x<endX; x++){
				...
				if(map[y][x].sd==0 && map[y][x].sdId >= 0){
					drawScaleCropImage(Img.mapShadow, Img.shadows[map[y][x].sdId].x, Img.shadows[map[y][x].sdId].y, Img.shadows[map[y][x].sdId].w, Img.shadows[map[y][x].sdId].h, x*mapTileWidth - PLAYERS[playerID].x + canvas.width/2, y*mapTileHeight  - PLAYERS[playerID].y + canvas.height/2, mapTileWidth, mapTileHeight);		
...
statsUpdate = function(){
	...
	for(i in PLAYERS){
		...
		c.fillText(PLAYERS[i].name, PLAYERS[i].x - PLAYERS[playerID].x + canvas.width/2, PLAYERS[i].y - PLAYERS[playerID].y + canvas.height/2+30); 
	}
...
gameLoop = function(){
	...
	if(connection.readyState == connection.OPEN && PLAYERS[playerID] != undefined){
		connection.send(JSON.stringify({header:"rot", data:{rot:playerRot,x:PLAYERS[playerID].x,y:PLAYERS[playerID].y}}));
		test.set(playerRot);
	}
	if(PLAYERS[playerID] != undefined)
		if(!PLAYERS[playerID].onAttack && onMouse)
			if(connection.readyState == connection.OPEN) connection.send(JSON.stringify({header:"sendMouse", data:""}));
},
...
window.onkeydown = window.onkeyup = function(event){
	...
	if(!chatting && (code==65 || code==37 || code==68 || code==39 || code==87 || code==38 || code==83 || code==40 || code==16)){
		connection.send(JSON.stringify({header:"sendKey", data:{
																"code":code,
																"status":keys[code]
															}}));
		if(PLAYERS[playerID] != undefined){												
